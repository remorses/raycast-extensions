...
function getOpenWindowIds(): { sessionId: string | null; windowIds: Set<number> } {
  try {
    const result = execFileSync(
      "sqlite3",
      [getPath(), "SELECT key, value FROM kv_store WHERE key IN ('session_id', 'session_window_stack')"],
      { encoding: "utf8" },
    );
...
export function useZedRecentWorkspaces(schemaVersion?: number, query?: string | null): ZedRecentWorkspaces {
  const path = getPath();
  const { sessionId, windowIds } = getOpenWindowIds();

  // Always call the hook, but use a dummy query if not ready
  const { data, isLoading, error, mutate, revalidate } = useSQL<Workspace>(
    path,
    query || "SELECT * FROM workspaces LIMIT 0",
  );
  ...
  return {
    entries: data
      ? data.reduce<ZedEntries>((acc, workspace) => {
          ...
          const isOpen =
            workspace.session_id === sessionId && workspace.window_id !== null && windowIds.has(workspace.window_id);

          return { ...acc, [entry.uri]: { ...entry, isOpen } };
        }, {})
      : {},
    isLoading,
    error,
